...
vector< vector<float> > normalize(vector< vector <float> > grid) {
    ...
    float sum = 0;
     ...
     for( int j = 0 ; j < grid[0].size() ; j++ ) {
  	...
  	for( int in = 0 ; in < grid.size() ; in++) {
     newRow.clear();
     for( int jn = 0 ;  jn < grid[0].size() ; jn++ ) {
      newRow.push_back( grid[in][jn] / sum); 
     }
     ...
     for( int j = 0 ; j <width ; j++ ) {
      ...
      for( int dx = -1 ; dx < 2 ; dx++ ) {
       for( int dy = -1 ; dy < 2 ; dy++ ) {
         multVal = window[dx+1][dy+1];
		 newi = (i + dy + height) % height;
		 newj = (j + dx + width) % width;
         newGrid[newi][newj] = newGrid[newi][newj] + ( multVal * gridVal ); 
       }
      }
     }
    }
